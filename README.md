...
- `wencai_sector_overrides.json`：可选的同花顺问财个股行业校正层；
- `wencai_enrichment.part*.b64`：分片存放的批量问财行业、ST、技术结构和财务增强数据。
...
`wencai_enrichment.part*.b64` 由问财原始 Excel 自动清洗、压缩并分片生成，股票代码会去除交易所后缀。系统按以下优先级合并：

1. 个股手工校正行业；
2. 同花顺二级行业，同时保留一级、三级和申万行业；
3. AKShare 或 BaoStock 自动行业。

ST 标记用于候选过滤；市值、PE、PB、ROE、营收增长和净利增长用于基本面分位评分。问财均线与量比只在导出日期当天覆盖云端技术结构，之后自动停止使用，避免把旧快照当作实时数据。问财价格只作为导入审计数据，不覆盖 AKShare/腾讯的最新行情。
